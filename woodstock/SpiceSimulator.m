...
- (id)init
{
	...
	cpu = spice_new_processor (NNPR_RAM);
...
- (BOOL)displayScan
{
	...
	int display_scan_position = SPICE_LEFT_SCAN;
	int display_position = 1;
	segment_bitmap_t seg0 = 0;
    BOOL need_update = NO;
	
	if (cpu->display_enable) {
		if (cpu->b [display_scan_position] & 4)
			seg0 = char_gen ['-'];
		if (seg0 != display_segments[0]) {
			need_update = YES;
			display_segments [0] = seg0;
		}
	}
    
	for (display_scan_position = SPICE_LEFT_SCAN; display_scan_position >= WOODSTOCK_RIGHT_SCAN; display_scan_position--) {
